#!/usr/bin/env bash
# install-tools.sh — Checks/installs the external tools this Neovim config
# needs. Symlinking ~/.config/nvim itself is handled by the parent dotfiles
# repo's own `install` script (ln -vs dotfiles/nvim .config/nvim); this
# script only covers the LSP/formatter/linter tooling around it.
set -euo pipefail
