    ...
    keys = {
      { "<leader>pv", "<cmd>VenvSelect<cr>", desc = "Select venv" },
      {
        ...
        function()
          ...
          vim.notify(python ~= "" and python or "No venv activated", vim.log.levels.INFO)
        end,
        desc = "Current venv",
      },
    },
    -- Default fd-based searches already discover uv-created .venv in the
    -- project root plus poetry/pyenv/etc.; caching + auto re-activation are
    -- on by default too, so no overrides are needed here.
    opts = {},
  },
